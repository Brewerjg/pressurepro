      ...
      <section className="mx-4 pb-3.5">
        <div className="relative">
          <select
            ...
            className="w-full h-11 pl-3 pr-9 rounded-xl border-[1.5px] border-neutral-200 bg-card text-sm font-medium outline-none focus:border-brand-800 transition-colors appearance-none"
          >
      ...
      {!isLoading && view.length === 0 && (
        <section className="mx-4">
          <div className="tp-card p-8 text-center">
            <div className="h-12 w-12 mx-auto rounded-2xl bg-brand-100 text-brand-800 flex items-center justify-center">
              <Camera className="h-6 w-6" strokeWidth={1.8} />
            </div>
            <h3 className="tp-display text-lg mt-3 text-neutral-900">No photos yet</h3>
            <p className="text-sm text-neutral-500 mt-1 max-w-[260px] mx-auto">
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            {view.map((p) => (
              <li key={p.id}>
                <Link
                  to={`/photos/${p.id}`}
                  className="block tp-card p-0 overflow-hidden active:bg-neutral-100 transition-colors"
                >
                  <div className="relative grid grid-cols-2 gap-px bg-hairline aspect-[2/1.05]">
                    ...
                    {p.afterUrl ? (
                      ...
                      <div className="bg-neutral-100 flex items-center justify-center text-[10px] font-bold uppercase tracking-wider text-neutral-500">
                        + After
                      </div>
                    )}
                    <span className="absolute top-2 left-2 px-2 py-0.5 rounded-full bg-neutral-900/80 text-white text-[9px] font-bold uppercase tracking-wider">
                      Before
                    </span>
                    <span className="absolute top-2 right-2 px-2 py-0.5 rounded-full bg-brand-800 text-white text-[9px] font-bold uppercase tracking-wider">
                      After
                    </span>
                  </div>
                  <div className="p-3">
