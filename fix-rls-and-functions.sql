-- Fix RLS policies and missing functions for TurfPro
...
-- 4. Create or update the current user's profile to ensure it exists
DO $$
BEGIN
    -- Try with id column first
    IF EXISTS (SELECT 1 FROM information_schema.columns
               WHERE table_name = 'profiles' AND column_name = 'id') THEN
        INSERT INTO profiles (id, created_at, is_demo, onboarded_at)
        VALUES (auth.uid(), NOW(), false, NULL)
    ...
    ELSIF EXISTS (SELECT 1 FROM information_schema.columns
                  WHERE table_name = 'profiles' AND column_name = 'user_id') THEN
        INSERT INTO profiles (user_id, created_at, is_demo, onboarded_at)
        VALUES (auth.uid(), NOW(), false, NULL)
...
SELECT 'RLS policies and functions fixed!' as status;